...
//			record.setId(mCursor.getInt(mCursor.getColumnIndex(DbAdapter.KEY_ROWID)));
    ...
    public void updateBmi(BMIBean bean) {
        db.execSQL("insert into bmi(weight,height,bmi,time) values(?,?,?,?)", new String[]{bean.getWeight(), bean.getHeight(), bean.getBmi(), bean.getTime()});
    }
    ...
    public void getLatesBmi() {
        db.execSQL("insert into bmi(weight,height,bmi,time) values(?,?,?,?)", new String[]{"100", "170", "20", "1472351171673"});
    }
